...
@section('content')
    <div class="my-10">
        <h1 class="my-4 text-3xl font-bold">EDIT DEPARTMENT</h1>
    ...
    <div class="grid grid-cols-1 space-y-5 md:grid-cols-5 md:space-x-10 md:space-y-0">
        ...
        <form method="POST" action="{{ route('department.update', $department->id) }}" id="form-employee"
            ...
            <div class="mb-5">
                ...
                <textarea id="description" rows="4" name="description"
                    class="{{ $errors->has('description') ? 'input-error' : 'input-default' }} block w-full rounded-lg border p-2.5 text-sm">{{ $department->description ?? old('description') }}</textarea>
                @error('description')
