...
@section('content')
    <div class="my-10">
        <h1 class="my-4 text-3xl font-bold">EDIT EMPLOYEE</h1>
        ...
        <form method="POST" action="{{ route('employee.update', $employee->id) }}" id="form-employee"
            ...
            <div class="mb-5">
                ...
                <input type="tel" name="phone" id="phone" aria-describedby="helper-text-explanation"
                    ...
                    value="{{ $employee->phone ?? old('phone') }}" pattern="^(?:\+62|62|0)8[1-9][0-9]{6,9}$" />
                @error('description')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-5">
                ...
                <input type="number" id="salary" name="salary" aria-describedby="helper-text-explanation"
                    ...
                    value="{{ number_format($employee->salary, 0, ',', '') ?? old('salary') }}" />
            ...
            <div class="mb-5">
                ...
                <div class="relative">
                    ...
                    <input datepicker id="hire_date" type="text" name="hire_date"
                        ...
                        placeholder="Select date" value="{{ $employee->hire_date ?? old('hire_date') }}">
                </div>
...
@push('after-script')
    <script type="module">
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            // Cek jika $employee->photo bernilai null
            let photoSource = "{{ Storage::disk('public')->url('images/employee/' . $employee->photo) }}";
            if (!{{ $employee->photo ? 'true' : 'false' }}) {
                // Jika photo bernilai null, tentukan nilai default atau kosongkan
                photoSource = null;
            }
